...
1. [为什么要从零手写一个 Agent？](#1-为什么要从零手写一个-agent)
2. [先破除两个常见误解](#2-先破除两个常见误解)
3. [最小 Agent 的六层结构](#3-最小-agent-的六层结构)
   - 第一层：模型调用层
   - 第二层：工具描述层
   - 第三层：工具执行层
   - 第四层：状态管理层
   - 第五层：停止条件
   - 第六层：可观测性
4. [Agent 的运行主循环](#4-agent-的运行主循环)
5. [从零实现一个最小 Agent](#5-从零实现一个最小-agent)
6. [真实系统里最常遇到的六类问题](#6-真实系统里最常遇到的六类问题)
7. [让系统从"能跑"到"可控"](#7-让系统从能跑到可控)
8. [什么时候手写，什么时候上框架](#8-什么时候手写什么时候上框架)
9. [总结](#9-总结)
