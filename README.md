...
- 评估与进化：作为系统优化与迭代闭环
- 本体论与知识表示：从哲学概念落到实体消歧、关系建模与跨系统语义对齐
- 自进化 Skills 与 Agent 自我改进：从人工预置技能走向自动发现、打包与自我修正的闭环
- 数据治理：Agent 时代的数据血缘、敏感隔离、审计留痕与合规落地
- Agent 实战案例：编程助手 Agent 等具体工程场景的实战拆解（原 docs/agent/ 游离目录正式接入导航）
...
- 新增 3 个模块骨架（目录 + 侧边栏 + 导航入口 + 模块首页）：
  ...
  - `08-self-evolving-skills/` 自进化 Skills 与 Agent 自我改进
  - `09-data-governance/` 数据治理
- 处理游离目录：将 `docs/agent/`（Agent 概念页 + 编程助手实战案例）正式接入顶部导航「实战案例」与侧边栏，不再是无入口的死目录
- 根目录新增 `ROADMAP.md` 选题看板，将 spec-v2 中 59 篇目标文章、阶段划分、待写篇目、组件激活与前端优化按优先级全部可视化，状态标记为「待写 / 草稿 / 已发布 / 待复核」
- 同步重写 `docs/.vitepress/config.mjs`：
  - 顶部 nav 新增「进阶主题」下拉（07/08/09 三模块）与「实战案例」下拉（Agent 实战）
  - `buildSidebar` 新增 `sidebarOntology` / `sidebarSelfEvolving` / `sidebarDataGovernance` / `sidebarAgent` 四个函数
  - 三个新模块骨架先挂 `模块概述` 一项，正文等内容到位后再挂
- 同步重写 `docs/index.md` 首页：
  - 「7 个模块」→「10 个模块」，「目标 52 篇」→「路线图看板追踪 59 篇」
  - 推荐阅读顺序从 5 步扩展为 7 步，新增进阶三模块 + Agent 实战的推荐路径
  - 知识地图更新为 10 主线 + 1 实战分支
  - 「七大模块」→「十大模块」卡片网格，补齐 07 / 08 / 09 三张新卡片与描述标签
  - 「当前内容进度」表新增三模块骨架记录，并标注 Memory/多 Agent/评估的待补篇数
- 同步更新 `docs/guide/index.md` 模块导航列表，新增 07/08/09 三模块简介与独立「实战入口」段落
- Python 教程与 models/ 下内容未动，Python 基础仍保留文件但不在主导航展示
- 所有现有正文内容均未改写，等待后续逐篇投喂后再增量更新或新增
- 保留阅读增强功能：侧边栏折叠、目录隐藏、专注阅读与状态记忆
- 统一了主题样式，按 `ai-agent-guide-spec-v2.md` 的中长期工程化治理方法论继续细化站点骨架
