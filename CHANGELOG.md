...
这个文件用于记录项目的重要结构调整、内容发布和工程变更。

后续如果有较大的内容更新、导航调整、模块迁移、案例新增或工程配置改动，请优先把记录写到这里，而不是继续堆在 [README.md](file:///Users/qian/Documents/workspace/ai-guide/README.md) 里。
...
- 新增 [DeerFlow Harness 深度拆解](file:///Users/qian/Documents/workspace/ai-guide/docs/agent/cases/deerflow-harness-deep-dive/index.md) 案例文章，并同时接入「实战案例」与「工具与框架」入口，避免它只挂在 `agent/cases` 下导致在 Harness 主线里不易发现
...
- 处理游离目录：将 `docs/agent/`（Agent 概念页和编程助手实战案例）正式接入顶部导航「实战案例」与侧边栏，不再是无入口的死目录
- 根目录新增 [ROADMAP.md](file:///Users/qian/Documents/workspace/ai-guide/ROADMAP.md) 选题看板，将 spec-v2 中 59 篇目标文章、阶段划分、待写篇目、组件激活与前端优化按优先级全部可视化，状态标记为「待写 / 草稿 / 已发布 / 待复核」
- 同步重写 [config.mjs](file:///Users/qian/Documents/workspace/ai-guide/docs/.vitepress/config.mjs)：
...
- 同步重写 [docs/index.md](file:///Users/qian/Documents/workspace/ai-guide/docs/index.md) 首页：
  - 「7 个模块」扩展为「10 个模块」
  - 「目标 52 篇」更新为「路线图看板追踪 59 篇」
  - 推荐阅读顺序从 5 步扩展为 7 步，新增进阶三模块与 Agent 实战推荐路径
  - 知识地图更新为 10 主线加 1 实战分支
  - 「七大模块」卡片网格扩展为「十大模块」
- 同步更新 [guide/index.md](file:///Users/qian/Documents/workspace/ai-guide/docs/guide/index.md) 模块导航列表，新增 07/08/09 三模块简介与独立「实战入口」段落
- Python 教程与 `models/` 下内容未动，Python 基础仍保留文件但不在主导航展示
- 所有现有正文内容均未改写，等待后续逐篇补充后再增量更新或新增
- 保留阅读增强功能：侧边栏折叠、目录隐藏、专注阅读与状态记忆
- 统一了主题样式，按 `ai-agent-guide-spec-v2.md` 的中长期工程化治理方法论继续细化站点骨架
- 重写 [config.mjs](file:///Users/qian/Documents/workspace/ai-guide/docs/.vitepress/config.mjs)，重建导航、侧边栏、本地搜索与主模块路由
- 重做 [docs/index.md](file:///Users/qian/Documents/workspace/ai-guide/docs/index.md)，将其改为 AI Agent 知识体系首页而非泛介绍页
- 首页进一步补齐了站点概览、推荐阅读顺序、模块进度和更完整的模块入口卡片
- 新增 [KnowledgeMap.vue](file:///Users/qian/Documents/workspace/ai-guide/docs/.vitepress/theme/components/KnowledgeMap.vue) 与 [ArticleHeader.vue](file:///Users/qian/Documents/workspace/ai-guide/docs/.vitepress/theme/components/ArticleHeader.vue) 主题组件，增强知识定位与文章上下文信息
- 新建 `00-preface` 到 `06-eval-evolution` 的模块首页与首批关键文章
- 统一强化 7 个模块首页结构，补齐模块定位、适合谁读、进入前建议、推荐顺序和后续跳转
...
- 已补齐 `从零手写 Agent` 与 `奖励函数设计` 两篇实现层和评估层关键正文，覆盖最小结构、常见误区、奖励组合与工程判断
...
- 已修复首页卡片与知识地图组件中的根路径写死问题，改为 base 感知链接，避免部署在子路径或代理域名下时跳转到错误地址
- 已将 `语言模型基础` 模块重构为连续的 `第1章` 到 `第10章` 小册子结构，系统覆盖 Transformer、Token、Embedding、位置编码、Self-Attention、QKV、矩阵视角、Block、Encoder、Decoder、BERT、GPT 与现代演化
...
- 已进一步修复 `chapter-03-self-attention-qkv` 中一处 Mermaid 非兼容节点语法：将 `[/ sqrt(d_k)]` 这类容易触发解析失败的写法改为普通节点文案，避免在 GitHub Pages 的 Chrome 环境出现运行时报错
- 已继续清理模型模块 Mermaid 节点中的高风险标签字符，特别是括号、斜杠、冒号与 `&` 等写法，以降低 `mermaid 10.9.5` 在 GitHub Pages 浏览器环境中的解析失败概率
- 已继续补强 `工具与框架` 模块中的 [LangGraph 原理](file:///Users/qian/Documents/workspace/ai-guide/docs/05-tools-frameworks/langgraph-principles.md) 与 [Spring AI 框架原理](file:///Users/qian/Documents/workspace/ai-guide/docs/05-tools-frameworks/spring-ai-framework.md)：
...
- 已把 `语言模型基础` 模块的章节编号统一重排为连续的 `第1章` 到 `第10章`，移除补丁式编号，并同步更新正文标题、模块页、专题导读与侧边栏
...
- 已继续修复 Mermaid 线上兼容问题，修正 [context-engineering.md](file:///Users/qian/Documents/workspace/ai-guide/docs/02-agent-core/context-engineering.md) 中一处 Mermaid 代码块缺少结束 fence 导致的 GitHub Pages 运行时报错
- 已将 `D:\workspace\superpowers-main\superpowers-main\skills` 整体导入到当前项目的 `.trae/skills/`，后续执行任务时优先判断这些项目级 skills 是否适合先调用
- 已新增 [.trae/skills/README.md](file:///Users/qian/Documents/workspace/ai-guide/.trae/skills/README.md)，记录 imported skills 的来源、当前清单与后续维护方式
